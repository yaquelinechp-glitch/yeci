...
export default function PartnerDashboard() {
  ...
  if (error) return <div className="text-white/60">{t('common.error')}</div>;
  if (!stats) return <div className="text-white/60">{t('common.loading')}</div>;

  const cards = [
    { label: t('partner.activeDeals'), value: stats.active_deals, color: 'border-aconso-500' },
    { label: t('partner.completedDeals'), value: stats.completed_deals, color: 'border-emerald-500' },
    { label: t('partner.totalRevenue'), value: `$${stats.total_revenue.toLocaleString()}`, color: 'border-purple-500' },
    { label: t('partner.commissionsEarned'), value: `$${stats.commissions_earned.toLocaleString()}`, color: 'border-orange-500' },
    { label: t('partner.coursesEnrolled'), value: stats.courses_enrolled, color: 'border-indigo-500' },
    { label: t('partner.completionRate'), value: `${stats.completion_rate}%`, color: 'border-accent-500' },
  ];

  const programCards = [
    { label: t('dash.openConflicts'), value: conflicts ? String(conflicts.open ?? 0) : '—', color: 'border-red-500', path: '/partner/conflicts' },
  ];

  return (
    <div className="animate-fade-in">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white">{t('partner.welcome')} {user?.first_name || user?.username || user?.contact_name || 'Partner'}</h1>
        <p className="text-white/70 mt-1">{t('partner.subtitle')}</p>
      </div>
